
import React from 'react';
import type { Reminder } from '../types';
import TrashIcon from './icons/TrashIcon';
import RepeatIcon from './icons/RepeatIcon';

interface ReminderItemProps {
  reminder: Reminder;
  deleteReminder: (id: string) => void;
}

const formatRepeatDays = (days: number[]): string => {
    if (!days || days.length === 0) return '';
    const dayMap = ['D', 'L', 'M', 'M', 'J', 'V', 'S'];
    // Sort days starting from Monday (1) to Sunday (0)
    const sortedDays = [...days].sort((a, b) => {
        const dayA = a === 0 ? 7 : a;
        const dayB = b === 0 ? 7 : b;
        return dayA - dayB;
    });
    return sortedDays.map(d => dayMap[d]).join(', ');
}

const ReminderItem: React.FC<ReminderItemProps> = ({ reminder, deleteReminder }) => {
  const formatDate = (dateString: string) => {
    const date = new Date(dateString + 'T00:00:00'); // Ensure date is parsed in local timezone
    return date.toLocaleDateString('es-ES', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
  };

  const isRepeating = reminder.repeatDays && reminder.repeatDays.length > 0;

  return (
    <li className="bg-gray-800 p-4 rounded-lg flex items-center justify-between transition-transform hover:scale-[1.02] duration-200">
      <div className="flex-grow min-w-0">
        <p className="text-white font-medium truncate">{reminder.text}</p>
        <p className="text-sm text-indigo-300">
          {isRepeating ? `Inicia: ` : 'Vence: '}
          {formatDate(reminder.dueDate)}
        </p>
        {isRepeating && (
            <div className="flex items-center space-x-2 text-xs text-gray-400 mt-1">
                <RepeatIcon className="h-4 w-4" />
                <span>Se repite: {formatRepeatDays(reminder.repeatDays!)}</span>
            </div>
        )}
      </div>
      <button
        onClick={() => deleteReminder(reminder.id)}
        className="p-2 ml-2 rounded-full text-gray-400 hover:bg-red-500/20 hover:text-red-400 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-colors flex-shrink-0"
        aria-label="Eliminar recordatorio"
      >
        <TrashIcon />
      </button>
    </li>
  );
};

export default ReminderItem;